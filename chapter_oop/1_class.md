...
## id - идентификатор объекта, is - ссылаются на один объект
...
В примере создаем 2 разных списка с одинаковым содержимым. Переменная a ссылается на объект с идентификатором 4292354496, переменная b - на другой список с идентификатором 4292357856.
...
Внутри метода `\_\_repr\_\_` мы вызвали метод `length` того же объекта и обращаемся к нему **self.length()**
